...
class App extends Component {
  ...
  removeFruit = (id) => {
    // schritt 1: index rausfinden über die id
        ...
        <table>
          ...
          <tbody>
          {
            this.state.fruits.map((fruit) => <tr key={fruit.id} style={{ backgroundColor: fruit.color }}>
              <td>{fruit.name}</td>
              <td>{fruit.color}</td>
              <td><button onClick={() => this.removeFruit(fruit.id)}>Remove</button></td>
              <td><button onClick={() => this.changeColor(fruit.id)}>New color</button></td>
            </tr>)
          }
          </tbody>
        </table>
